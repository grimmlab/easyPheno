...
def objective(trial, device, data_path, outer_fold_number, k_folds=3, n_epochs=250, l1_loss_added=False, pca=False):
    ...
    loss_total = np.ones(k_folds) * 99999999
    ...
    path = f"mlp_regr_pytorch_checkpoint/{trial.number}"
    ...
    for k in range(k_folds):
        ...
        for epoch in range(epoch_begin, n_epochs):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                loss = loss_fn(output.float(), torch.reshape(target, (-1, 1)).float())
                if l1_loss_added:
                    reg_loss = 0
            ...
            with torch.no_grad():
                ...
                loss = mse  # + l1_lambda * reg_loss.item()
                print('MSE: ' + str(mse))
                print('Loss: ' + str(loss))
                if loss < loss_total[k]:
                    print('new best')
                    loss_total[k] = loss
                    epochs_no_improve = 0
                    ...
                    results.to_csv(
                        ...
                        + str(outer_fold_number) + '_' + str(k) + '_'
...
def restart_from_checkpoint(study, trial):
    ...
    path = f"mlp_regr_pytorch_checkpoint/{trial.number}/model.pt"
...
if __name__ == '__main__':
    ...
    for outer_fold_number in [0]: #[0, 1, 2, 3 4]
        ...
        db_name = 'mlp_regr' + str(n_trials) + '-' + str(k_folds) + '-' + str(n_epochs) + str(l1_loss_added) \
